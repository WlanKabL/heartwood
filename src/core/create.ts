...
export const createNode = async (
  ...
  // Advisory hints are best-effort: if any step throws (e.g. a DB hiccup in searchNodes),
  // the node is already persisted so we must always return a success result.
  ...
  try {
    ...
    volatilityWarning = detectVolatility(input.content) ?? undefined
    ...
    // Advisory hints are non-blocking. A successful insert always returns the created node.
  }
